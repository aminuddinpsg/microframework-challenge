...
class TestInterFileService(unittest.TestCase):
    ...
    def test_savePictureWrongFile(self):
        ...
        files = {'file': file}
        response = requests.post('http://localhost:8080/picture', files=files)
    ...
    def test_savePictureEmpty(self):
        ...
        response = requests.post('http://localhost:8080/picture', files=None)
    ...
    def test_saveZip(self):
        ...
        files = {'file': file}
        response = requests.post('http://localhost:8080/zip', files=files)
    ...
    def test_saveThumbnailSmall(self):
        ...
        files = {'file': file}
        response = requests.post('http://localhost:8080/thumbnail', files=files)
        self.assertIsNotNone(response.json()['url'])
        file.close()
